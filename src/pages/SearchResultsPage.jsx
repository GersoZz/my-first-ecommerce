...
function SearchResultsPage() {
  ...
  const searchUrl = query ? `${API_ENDPOINTS.PRODUCTS}/?categorySlug=${encodeURIComponent(query)}` : null

  const { data: productsRawData, isLoading, error } = useFetch(searchUrl)

  const products = productsRawData ? productsRawData.slice(0, 12).map(productsAdapter) : []
